...
const ledgerApp = (() => {
    ...
    // Element references
    const ledgerContent = document.getElementById('lab-ledger-content');
    const addReceiveModal = document.getElementById('addReceiveModal');
    const testUsageModal = document.getElementById('testUsageModal');
    ...
    const handleModalClose = (modalId) => {
        ...
        if (form) form.reset();
        if (modalId === 'testUsageModal') document.getElementById('chemical-entry-container').innerHTML = '';
    };
    ...
    async function init() {
        ...
        // *** THE FIX: Attach listeners to specific containers ***
        ledgerContent.addEventListener('input', e => {
            const target = e.target;
            if (target.id === 'historyFilter') {
                renderUsageHistory(target.value);
            }
            if (target.id === 'inventorySearch') {
                renderInventory(target.value);
            }
        });
        ...
        ledgerContent.addEventListener('click', async (e) => {
            ...
            if (target.classList.contains('delete-inventory-btn')) {
                ...
                try {
                    ...
                    alert("Failed to delete chemical. See console for details.");
                }
            }
            ...
            if (target.classList.contains('delete-history-btn')) {
                ...
                if (chemical && t) {
                    ...
                    alert("Could not find the original chemical batch to restore stock to. The batch may have been deleted.");
                }
            }
        });
        ...
        addReceiveModal.querySelector('form').addEventListener('submit', async (e) => {
             ...
             try {
                 ...
                 handleModalClose('addReceiveModal');
        ...
        testUsageModal.querySelector('form').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                await batch.commit();
                handleModalClose('testUsageModal');
        ...
        editChemicalModal.querySelector('form').addEventListener('submit', async(e) => {
            ...
            if(chemical){
                try {
                    ...
                    handleModalClose('editChemicalModal');
        ...
        editHistoryModal.querySelector('form').addEventListener('submit', (e) => {
             ...
             handleModalClose('editHistoryModal');
        });
    }

    return { init, getInventory: () => inventory, handleModalClose };
})();