...
const reportApp = (() => {
    ...
    // Element references
    const dailyActivityContent = document.getElementById('daily-activity-content');
    const sterilityLogContent = document.getElementById('sterility-log-content');
    const reportDateInput = document.getElementById('report-date');
    const historySelect = document.getElementById('history-select');
    const autocompleteBox = document.getElementById('autocomplete-box');
    const sterilityModal = document.getElementById('sterility-modal');
    const sterilityForm = document.getElementById('sterility-form');
    ...
    function init() {
        ...
        const handleClick = async (e) => {
            ...
            switch (action) {
                ...
                case 'close-modal': 
                    ...
                    if (modal && modal.id === 'sterility-modal') {
        ...
        // Attach event listeners to the specific parent divs
        ...
        sterilityForm.addEventListener('submit', async (e) => {
            ...
            try { 
                if (id) { 
                    await db.collection('sterilityLog').doc(id).set(record, { merge: true }); 
                } else { 
                    await db.collection('sterilityLog').add(record); 
                } 
                sterilityModal.style.display = 'none'; 
            } catch (err) { 
                console.error("Error saving sterility record:", err); 
            }
        });
        ...
        document.addEventListener('keydown', (e) => { if (e.key === 'Escape') hideAutocomplete(); }); // This can stay global
...
// Global function for printing Daily Report
